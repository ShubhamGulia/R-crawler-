...
frontier <- c("http://www.cnn.com","http://news.google.com","http://www.kdnuggets.com")
...
while (j < num){
  ...
  doc <- tryCatch(exploredlink,error=function(cond){return("")})
  
  if(str_length(doc)<10){
    next
  }
  
  response<- GET(doc,path="/",query=c(seq_condenacao=as.character(i)))
  doc <- htmlParse(response, encoding = "UTF-8")
  #doc <- htmlParse(doc)
  #doc <- htmlParse(rawToChar(GET(doc)$content))
  #htmlContent <- getURL(doc)
  ...
  titleText <- c(unique(titleText))
  titleText = tolower(titleText)
  # toString(titleText)
  l = typeof(titleText)
  p = typeof(topicwords)
  #print(doc)
  #print(titleText)
  for(i in 1:length(topicwords)){
    if(grepl(topicwords[i],titleText))
      print(domain)
  }
